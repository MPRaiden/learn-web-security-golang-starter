...
const serviceDefaultTimeout = 5 * time.Second
...
func NewService() *Service {
	transport := &http.Transport{ResponseHeaderTimeout: serviceDefaultTimeout, TLSHandshakeTimeout: serviceDefaultTimeout, Proxy: nil}
	return &Service{client: &http.Client{Transport: transport, Timeout: serviceDefaultTimeout, CheckRedirect: func(_ *http.Request, _ []*http.Request) error { return http.ErrUseLastResponse }}}
}
...
func allowedURL(rawURL string) (*url.URL, bool) {
	...
	if err != nil || parsed.Host != "storage.googleapis.com" || parsed.Scheme != "https" || parsed.User != nil {
		return nil, false
	}
	if parsed.Path == "" {
		parsed.Path = "/"
