...
func (store *Store) VerifyAndConsume(ctx context.Context, userID int64, code, secret string) (bool, error) {
	...
	verified := verifyAt(code, secret, store.now())
	if !verified {
		return false, nil
	}
	currentStep := currentTime.Unix() / totpPeriodSeconds

	sqlResult, err := store.queries.ConsumeTOTPStep(ctx, dbgen.ConsumeTOTPStepParams{TimeStep: &currentStep, UserID: userID})
	if err != nil {
		return false, err
	}
...
func (store *Store) ConsumeBackupCode(ctx context.Context, userID int64, code string) (bool, error) {
	sqlResult, err := store.queries.ConsumeTOTPBackupCode(ctx, dbgen.ConsumeTOTPBackupCodeParams{UserID: userID, CodeHash: hashToken(code)})
	if err != nil {
		return false, fmt.Errorf("consume TOTP backup code: %w", err)
	}
	rowsAffected, err := sqlResult.RowsAffected()
	if err != nil {
		return false, fmt.Errorf("sql result rows affected: %w", err)
	}
	if rowsAffected != 1 {
		return false, nil
	}
